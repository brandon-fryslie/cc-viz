...
import { useSearchPlans } from '../lib/api'
import { AppLayout } from '../components/layout'

export function PlansSearchPage() {
  ...
  const { data: searchResults, isLoading, isError, error } = useSearchPlans(
    debouncedSearch,
    statusFilter,
    50,
    offset
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="px-4 py-2 bg-[var(--color-bg-secondary)] border-b border-[var(--color-border)] text-sm text-[var(--color-text-secondary)]">
          {isLoading ? (
            ...
            <span>
              Showing {plans.length} plan{plans.length !== 1 ? 's' : ''} of {total}
            </span>
          ) : (
        ...
        <div className="flex-1 overflow-auto">
          {plans.length === 0 && !isLoading ? (
            <div className="p-4 text-center text-[var(--color-text-muted)]">
              {debouncedSearch ? 'No plans match your search' : 'Enter a search query'}
            </div>
          ) : (
            <table className="w-full">
              <thead className="bg-[var(--color-bg-hover)] sticky top-0">
                <tr>
                  ...
                  <th className="px-4 py-2 text-left text-[var(--color-text-primary)]">Title</th>
                  <th className="px-4 py-2 text-left text-[var(--color-text-primary)]">Goal</th>
                </tr>
              </thead>
              <tbody>
                {plans.map((plan: any) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-2 font-medium text-[var(--color-text-primary)] truncate max-w-md">
                      {plan.title}
                    </td>
                    <td className="px-4 py-2 text-[var(--color-text-secondary)] truncate max-w-md">
                      {plan.goal}
                    </td>
                  </tr>
                ))}
