...
function ConversationDetailPane({ conversationId }: { conversationId: string | null }) {
  ...
  // P8: Apply search highlighting when detail content loads
  useSearchHighlight(
    (!!conversation && !isLoading) || (useDBMessages && !!dbMessagesResponse && !isLoadingDBMessages),
    detailContainerRef
  )
...
export function ConversationsPage() {
  ...
  // When searching, use FTS results; otherwise use client-side filtering for project names
  const filteredConversations = useMemo(() => {
    // Use backend FTS search results when available (query >= 2 chars)
    if (searchQuery.length >= 2 && searchResults?.results) {
      // Convert search results to Conversation format and match with full data
      const searchIds = new Set(searchResults.results.map(r => r.conversationId))
      const matched = sortedConversations.filter(c => searchIds.has(c.id))

      // Sort by match count (most relevant first) while preserving search result order
      const matchCountMap = new Map(searchResults.results.map(r => [r.conversationId, r.matchCount]))
      return matched.sort((a, b) => (matchCountMap.get(b.id) || 0) - (matchCountMap.get(a.id) || 0))
    }

    // For short queries (< 2 chars), use client-side filtering on project names only
  ...
  }, [sortedConversations, searchQuery, searchResults])
