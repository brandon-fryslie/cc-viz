...
function RequestDetail({ requestId }: { requestId: string | null }) {
  ...
  useSearchHighlight(!!request && !isLoading, detailContainerRef)
  ...
  return (
    <div ref={detailContainerRef} className="p-4 h-full overflow-auto">
...
export function RequestsPage() {
  ...
  const { data: requests, isLoading: isLoadingRequests, refetch } = useRequestsSummary({
    model: modelFilter === 'all' ? undefined : modelFilter
  })

  // Search requests with full-text search
  const { data: searchResults, isLoading: isLoadingSearch } = useSearchRequests(
    searchQuery,
    {
      model: modelFilter === 'all' ? undefined : modelFilter,
      limit: 50,
  ...
  // Use server-side search results if searching, otherwise use regular list
  const isSearching = searchQuery.length >= 2 && searchResults
  const displayRequests: (RequestSummaryType | RequestSearchResult)[] = isSearching ? (searchResults?.results || []) : (requests || [])
  const isLoading = isSearching ? isLoadingSearch : isLoadingRequests
                ...
                >
                  ...
                  >
                    {virtualizer.getVirtualItems().map((virtualItem) => {
                      const request = displayRequests[virtualItem.index]
                      ...
                      return (
                        ...
                        >
                          <RequestListItem
                            ...
                            isCompareSelected={compareState.selectedIds.includes(requestId)}
                            onCompareToggle={() => toggleRequestSelection(requestId)}
                          />
                        </div>
                      )
                    })}
