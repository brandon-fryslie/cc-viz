...
func extractRequestTextForFTS(request *model.RequestLog) (toolNames, responseText, requestBodyText string) {
	// Extract tool names
	if len(request.ToolsUsed) > 0 {
		toolNames = strings.Join(request.ToolsUsed, ", ")
	}

	// Extract response text (body preview, first 1000 chars)
	if request.Response != nil {
		if request.Response.BodyText != "" {
			// Use BodyText if available
			if len(request.Response.BodyText) > 1000 {
				responseText = request.Response.BodyText[:1000]
			} else {
				responseText = request.Response.BodyText
			}
		} else if len(request.Response.Body) > 0 {
			// Try to use raw body
			bodyStr := string(request.Response.Body)
			if len(bodyStr) > 1000 {
				responseText = bodyStr[:1000]
			} else {
	...
	// Extract request body text (if available, first 1000 chars)
	if request.Body != nil {
		bodyJSON, err := json.Marshal(request.Body)
		if err == nil {
			bodyStr := string(bodyJSON)
			if len(bodyStr) > 1000 {
				requestBodyText = bodyStr[:1000]
			} else {
				requestBodyText = bodyStr
			}
		}
	}
...
func (s *SQLiteStorageService) SaveRequest(request *model.RequestLog) (string, error) {
	...
	// Index request in FTS5 for full-text search
	if fts5Enabled() {
		toolNames, responseText, requestBodyText := extractRequestTextForFTS(request)
		if err := s.indexRequestFTS(
			request.RequestID,
			request.Timestamp,
			request.Method,
			request.Endpoint,
			...
			// Log error but don't fail the request save
			log.Printf("⚠️ Warning: failed to index request %s in FTS: %v", request.RequestID, err)
		}
	}
...
func (s *SQLiteStorageService) SearchRequests(query string, modelFilter string, limit, offset int, after, before string) (*model.RequestSearchResults, error) {
	...
	for rows.Next() {
		...
		// Extract snippet with highlights using ExtractSnippet
		if responseText.Valid && responseText.String != "" {
			snippet := ExtractSnippet(responseText.String, query, 100)
