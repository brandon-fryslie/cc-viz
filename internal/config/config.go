...
// QueueConfig holds file-based queue configuration
type QueueConfig struct {
	Directory       string `yaml:"directory" json:"directory"`                 // Queue directory path
	DeadLetterDir   string `yaml:"dead_letter_dir" json:"dead_letter_dir"`     // Directory for malformed files
	WatchInterval   string `yaml:"watch_interval" json:"watch_interval"`       // Interval for polling (if fsnotify unavailable)

	// Parsed interval duration
	WatchIntervalDuration time.Duration `yaml:"-" json:"-"`
}
...
func Load() (*Config, error) {
	...
	cfg := &Config{
		...
		Queue: QueueConfig{
			Directory:             "./queue",
			DeadLetterDir:         "./queue/dead-letter",
			WatchInterval:         "1s",
			WatchIntervalDuration: time.Second,
		},
	...
	if envQueueDir := os.Getenv("QUEUE_DIRECTORY"); envQueueDir != "" {
		cfg.Queue.Directory = envQueueDir
	}
	if envDeadLetterDir := os.Getenv("QUEUE_DEAD_LETTER_DIR"); envDeadLetterDir != "" {
		cfg.Queue.DeadLetterDir = envDeadLetterDir
	}
	...
	// Parse queue watch interval
	if cfg.Queue.WatchInterval != "" {
		if duration, err := time.ParseDuration(cfg.Queue.WatchInterval); err == nil {
			cfg.Queue.WatchIntervalDuration = duration
		}
	}
